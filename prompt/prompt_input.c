...
/*   Updated: 2024/07/30 14:49:07 by vvobis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
											...
											uint32_t prompt_length_current)
{
	...
	do_refresh = false;
	prompt_buffer_size_manage(input, prompt_length_current, 1);
												...
												uint32_t prompt_length_current)
{
	...
	prompt_buffer_size_manage(input, prompt_length_current, ft_strlen(buffer) + 1);
							...
							uint32_t cursor_position_base)
{
	...
	{	
		...
		{
			do_refresh = true;
			if (bytes_read > 10 && ignore_newline == false)
			{
